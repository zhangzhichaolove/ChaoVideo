...
import com.app.chao.chaoapp.bean.Collection;
import com.app.chao.chaoapp.bean.Record;
...
public class RealmHelper {
    ...
    public static final String DB_NAME = "chao.realm";
    ...
    public void insertSearchHistory(SearchKey bean) {
        ...
        if (list == null || list.size() == 0) {//没有重复元素-添加
            getRealm().beginTransaction();
            getRealm().copyToRealm(bean);
            getRealm().commitTransaction();
        } else {//有重复元素-替换插入时间
            getRealm().beginTransaction();
            bean.insertTime = System.currentTimeMillis();
